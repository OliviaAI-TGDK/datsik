...
from trainer import zengardenTrainer
...
# --- Resolve tokenized paths ---
BASE_DIR = Path(__file__).resolve().parent
PACKS_DIR = BASE_DIR / "packs" / "tokenized"
train_path = PACKS_DIR / "train"
val_path   = PACKS_DIR / "val"

if not train_path.is_dir():
    raise FileNotFoundError(f"[zengarden] ❌ Train dataset not found at: {train_path}")

if not val_path.is_dir():
    print("[zengarden] ⚠️ Validation set missing; using train split as validation.")
    val_path = train_path
...
if train_dataset is None:
   # Convert to string for glob
    arrow_files = glob.glob(os.path.join(tokenized_dir.as_posix(), "*.arrow"))
...
class TGDKQuadCallback(TrainerCallback):
    ...
    def __init__(self,
        ...
        self.vault_log = self.out_dir / "TGDKVault_zengardenLog.ndjson"
...
try:
    ...
    print(f"[MirrorBlade] ⚠️ Duosync block failed: {e}")

    # ---------------------------------------------------------------------
# TGDK Gradient Checkpointing Compatibility Patch
# ---------------------------------------------------------------------
    def ensure_gradient_checkpointing_stub(model):
        """
        Ensures model has .gradient_checkpointing_enable() and .gradient_checkpointing_disable().
        Prevents HF Trainer from crashing when these are missing.
        """
        if not hasattr(model, "gradient_checkpointing_enable"):
            def _enable(*a, **k):
                print("[TGDK] ⚙️ gradient_checkpointing_enable (noop stub).")
            model.gradient_checkpointing_enable = _enable

        if not hasattr(model, "gradient_checkpointing_disable"):
            def _disable(*a, **k):
                print("[TGDK] ⚙️ gradient_checkpointing_disable (noop stub).")
            model.gradient_checkpointing_disable = _disable
...
if not os.path.exists(train_path):
    raise FileNotFoundError(f"[zengarden] ❌ Missing tokenized train dataset → {train_path}")
if not os.path.exists(val_path):
    print("[zengarden] ⚠️ No validation set detected — using train split as validation.")
    val_path = train_path
...
# --- TGDK patch for Trainer compatibility ---
if not hasattr(fusion_model, "gradient_checkpointing_enable"):
    fusion_model.gradient_checkpointing_enable = lambda *a, **k: print("[TGDK] ⚙️ gradient_checkpointing_enable (noop)")
if not hasattr(fusion_model, "gradient_checkpointing_disable"):
    fusion_model.gradient_checkpointing_disable = lambda *a, **k: print("[TGDK] ⚙️ gradient_checkpointing_disable (noop)")
...
fusion_model.name_or_path = "./tgdk_base_tok"
tgdk_tok.name_or_path = "./tgdk_base_tok"
from pathlib import Path

# --- make sure a valid local path exists ---
fallback_path = Path("./tgdk_base_tok").resolve()
fallback_path.mkdir(parents=True, exist_ok=True)

cfg_file = fallback_path / "config.json"
...
# --- force every component to use it ---
fusion_model.name_or_path = str(fallback_path)
tgdk_tok.name_or_path = str(fallback_path)
kwargs = dict(processing_class=None)          # TRL shouldn’t touch the hub
print(f"[TGDK] 💠 Safe fallback path enforced → {fusion_model.name_or_path}")
...
for name in ("train_dataset", "val_dataset"):
    ...
    if obj is not None:
        try:
            print(f"  len={len(obj)}")
            print(f"  first sample keys={list(obj[0].keys())}")
        except Exception as e:
...
try:
    ...
    raise ImportError("[zengarden] ❌ transformers.trainer_callback not found — please upgrade transformers.")
...
if args_ref is None:
    print("[zengarden] ⚠️ Trainer exposes no args — creating synthetic TrainingArguments container.")
...
if not hasattr(trainer, "optimizer") or trainer.optimizer is None:
    print("[zengardenTrainer] ⚙️ Building optimizer before callback handler...")
    try:
        trainer.create_optimizer_and_scheduler(num_training_steps=1)
        print("[zengardenTrainer] ✅ Optimizer and LR scheduler initialized early.")
    except Exception as e:
        print(f"[zengardenTrainer] ⚠️ Early optimizer build failed: {type(e).__name__}: {e}")
...
try:
    ...
    print(f"[zengarden] ❌ Failed to build CallbackHandler: {type(e).__name__}: {e}")
...
try:
    ...
    print(f"[zengarden] ❌ Callback setup failed: {type(e).__name__}: {e}")
...
trainer.fit(epochs=1, batch_size=1, grad_accum=1, checkpoint_path="./tgdk_zengarden.ckpt")
...
# === TGDK zengarden FINALIZATION ===
import os, json, hashlib, binascii, torch
from pathlib import Path
from cryptography.hazmat.primitives import serialization, hashes
from cryptography.hazmat.primitives.asymmetric import padding

print("[zengarden] 🔒 Entering deterministic finalization sequence...")

ARTIFACTS_DIR = Path("./artifacts")
KEYS_DIR = Path("./zengarden_keys")
SIGNERS = ["q0", "q1", "q2", "q3"]
...
if not checkpoints:
    print("[zengarden] ⚠️ No .pt checkpoints found; nothing to sign.")
else:
    for ck in checkpoints:
        print(f"[zengarden] 🧩 Signing {ck.name}")
        blob = ck.read_bytes()
        ...
        for name in SIGNERS:
            ...
            if not key_path.exists():
                print(f"[zengarden] ⚠️ Missing key {key_path}, skipping signer.")
                continue
        ...
        print(f"[zengarden] ✅ Signatures written to {sig_file.name}")
        ...
        for name in SIGNERS:
            ...
            if not verify_blob(blob, binascii.unhexlify(sig_hex), pub):
                print(f"[zengarden] ❌ Verification failed for {name}")
                all_ok = False
        if all_ok:
            print(f"[zengarden] ✅ Verification OK for {ck.name}")
        else:
            print(f"[zengarden] ⚠️ Verification mismatch detected for {ck.name}")

    print("\n[DΞT-SΞC] zengarden deterministic signature sequence complete.\n")

# === optional CI hook ===
if os.getenv("TGDK_CI") == "1":
    print("[zengarden] ⛓️ CI mode enabled — exporting signatures for attestation.")
    import subprocess
    subprocess.run(["gh", "workflow", "run", "zengarden_signer.yml"], check=False)
